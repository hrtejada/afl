...
for crash in $DIR/crashes/id_*; do
  ...
  if [ "$use_stdio" = "1" ]; then  
    $GDB --batch -q --ex "r $use_args <$crash" --ex 'back' --ex 'disass $pc, $pc+64' --ex 'info reg' --ex "generate-core-file gdb/cores/$id" --ex 'quit' "$BIN" 0</dev/null
  else
    $GDB --batch -q --ex "r $use_args" --ex 'back' --ex 'disass $rsp, $rbp' --ex 'info reg' --ex 'quit' "$BIN" 0</dev/null
  fi
  echo
